...
However... the `Data.Map` Dijkstra solution is pretty slow... in fact it's been running for at least 10 minutes now. It would have been smarter to use a `UArray` here. Might rewrite it later, but surely I have to let it finish now... surely... it will halt...

__Some time later__

Okay that wasn't really working. I rewrote it using `Data.Array` and... it was still slow. Really, really slow. The main slowness was the `getClosest` function, which walks over the entire weight array. I let it run and cook the room while I worked on an implementation of [leftist heaps](https://en.wikipedia.org/wiki/Leftist_tree) which would allow for a much more efficient priority queue (the slowest part of my Dijkstra implementation). Eventually it produced the correct answer after almost 2 hours, literally not cool.
...
When I ran it on the full input data, it produced the correct result again, in a delightful time of 3.3 seconds. That's 175 times faster than my Dijkstra implementation with leftist heaps. Wow.
...
|--------------|-----------------------------------------------------------------------|--------------|
| Dijkstra     | immutable Data.Map for distance table + vanilla lists as vertex queue | 2 hours      |
| Dijkstra     | immutable array for distance table + binary heap as vertex queue      | 10 minutes   |
| Bellman-Ford | mutable arrays for everything                                         | 3.3 seconds  |
| Dijkstra     | mutable array for distance table + sets as vertex queue               | 1 second     |
| Dijkstra     | mutable array for distance table + binary heap as vertex queue        | 0.95 seconds |
