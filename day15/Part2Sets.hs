...
-- algorithm with sets as a priority queue.
...
initialQ w h = S.fromList [(10000000, (i,j)) | i <- [0..w-1], j <- [0..h-1]]

solve' :: Array (Int, Int) Int -> STUArray s (Int, Int) Int -> S.Set (Int, Coord) -> Int -> Int -> ST s Int
solve' ws ds q w h | S.null q = readArray ds (w-1,h-1)
solve' ws ds q w h = do
  ...
  let qUpdated = foldr (\(u,ud') acc -> S.insert (ud',u) acc) q' newDs
...
getClosest :: WeightQ -> (Coord,WeightQ)
